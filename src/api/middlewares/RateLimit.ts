...
import { RedisCache } from "../../util/redis/Redis";

export const API_PREFIX_TRAILING_SLASH = /^\/api(\/v\d+)?\//;

// Docs: https://discord.com/developers/docs/topics/rate-limits

// Shared rate-limit counting across cluster workers/processes via Redis (Upstash),
// with an in-process Map as the fallback when Redis isn't configured. This fixes
// rate limits being per-worker-process under multi-process clustering (THREADS>1).
...
async function hitRoute(opts: { executor_id: string; bucket_id: string; max_hits: number; window: number }) {
    ...
    // Redis is the shared source of truth across cluster workers/processes.
    // If Redis isn't configured, incrRateLimit() returns 0 and we fall back
    // to the plain in-process counter below (same as before Redis existed).
    const sharedCount = await RedisCache.incrRateLimit(id, opts.window);
    limit.hits = sharedCount > 0 ? sharedCount : limit.hits + 1;
